...
public class Publisher extends JFrame {
	...
	public Publisher() {
		...
		lblNewLabel.setBounds(126, 13, 216, 37);
		...
		lblNewLabel_2.setBounds(47, 143, 81, 16);
		...
		add.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					PreparedStatement pst1 = con.prepareStatement("Select*from publisher where name=?");
					...
					if (rs.next() == false) {
						...
						PreparedStatement pst = con
								.prepareStatement("insert into publisher(`name`,`add`,`Phone`)" + "values(?,?,?)");
						...
						if (k == 1) {
							JOptionPane.showMessageDialog(null, "Publisher Created");
							txtname.setText("");
							txtadd.setText("");
							txtphone.setText("");
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					PreparedStatement pst1 = con
							.prepareStatement("Select*from publisher where name=? and `add`=? and Phone=?");
					pst1.setString(1, name);
					...
					if (rs.next() == false) {
						PreparedStatement pst = con
								.prepareStatement("update publisher set `name`=?,`add`=?,`Phone`=? where `ID`=?");
						pst.setString(1, name);
						pst.setString(2, address);
						pst.setString(3, phone);
						pst.setInt(4, ID);
		...
		delete.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					Connection con = DB.getConnection();
					PreparedStatement pst = con.prepareStatement("delete from publisher where ID=?");
		...
		delete.setBounds(279, 427, 97, 37);
		...
		back.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				Publisher.this.setVisible(false);
		...
		scrollPane.setBounds(457, 37, 671, 468);
		...
		lblNewLabel_3.setBounds(47, 245, 97, 16);
